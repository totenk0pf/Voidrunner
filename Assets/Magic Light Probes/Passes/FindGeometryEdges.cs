...
namespace MagicLightProbes
{
    ...
    {
        public IEnumerator ExecutePass(MagicLightProbes parent, float volumeSpacing, MLPVolume currentVolume)
        {
            ...
            for (int i = 0; i < parent.tmpNearbyGeometryPoints.Count; i++)
            {
                ...
                for (int d = 0; d < directions.Length && !edgeFound; d++)
                {
                    ...
                    {                        
                        ...
                        {
                            if (!parent.CheckIfInside(currentVolume.transform, parent.tmpNearbyGeometryPoints[i].position))
                            {
                                break;
                            }
                            ...
                            Ray ray = new Ray(parent.tmpNearbyGeometryPoints[i].position,
                                -parent.tmpNearbyGeometryPoints[i].collisionNormal);
