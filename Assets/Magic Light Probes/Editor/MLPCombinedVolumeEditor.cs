                ...
                if (GUILayout.Button("Mark As Bottom"))
                {
                    ...
                    for (int i = 0; i < sceneVolumes.Length; i++)
                    {
                        if (Selection.objects.Length > 1)
                        {
                            ...
                            {
                                if (!sceneVolumes[i].groundAndFloorObjects.Contains(Selection.objects[j].name))
                                {
                                    sceneVolumes[i].groundAndFloorObjects.Add(Selection.objects[j].name);
                                }
                            }
                        }
                        else
                        {
                            if (!sceneVolumes[i].groundAndFloorObjects.Contains(Selection.activeGameObject.name))
                            {
                                if (Selection.activeGameObject.GetComponent<MeshRenderer>() != null || Selection.activeGameObject.GetComponent<Collider>() != null)
                                {
                                    ...
                                    if (isStatic)
                                    {
                                    sceneVolumes[i].groundAndFloorObjects.Add(Selection.activeGameObject.name);
                    ...
                    for (int i = 0; i < sceneVolumes.Length; i++)
                    {
                        if (Selection.objects.Length > 1)
                        {
                            for (int j = 0; j < Selection.objects.Length; j++)
                            {
                                if (sceneVolumes[i].groundAndFloorObjects.Contains(Selection.objects[j].name))
                                {
                                    sceneVolumes[i].groundAndFloorObjects.Remove(Selection.objects[j].name);
                                }
                            }
                        }
                        else
                        {
                            if (sceneVolumes[i].groundAndFloorObjects.Contains(Selection.activeGameObject.name))
                            {
                                sceneVolumes[i].groundAndFloorObjects.Remove(Selection.activeGameObject.name);
